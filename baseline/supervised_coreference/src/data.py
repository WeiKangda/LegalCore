...
class Document:
    ...
    def populate_event_spans(self):
        ...
        event2id = {e["singleton_id"]:idx for idx, e in enumerate(events)} # sorted events to index
        self.id2event = {idx: e for idx, e in enumerate(events)}
        self.label_groups = [[event2id[e["singleton_id"]] for e in events] for events in self.merged_events] # List[List[int]] each sublist is a group of event index that co-references each other
...
class myDataset(Dataset):
    ...
    def load_examples(self, data_dir, split):
        ...
        for line in lines:
            ...
            doc = Document(data)
            id2event_dict[doc.id] = doc.id2event
...
if __name__ == "__main__":
    ...
    dataset = myDataset(tokenizer, "./data", "train", max_length=128)
